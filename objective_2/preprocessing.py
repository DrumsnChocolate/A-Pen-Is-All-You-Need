...
def read_and_pickle_data():
    ...
    person_folders = [f.path for f in os.scandir(super_folder) if f.is_dir() and (re.match('.*idea.*', f.path) is None)]
    ...
    log('\nPickling the data...')
    pickle.dump(all_persons_data, open(super_folder + pkl_filename, 'wb'))

def unpickle_data():
    with open(super_folder + pkl_filename, 'rb') as f:
        ...
        log(f'person {person_num}: {len(training_equations)} training equations, {len(adaptation_equations)} adaptation equations')
...
def plot_data(data):
    for person in range(len(data)):
        log(f'person {person}: {len(data[person])} equations')
    log(data[0][0][1]['Millis'])
    person0_eq0 = data[0][0]
    person0_eq0_eq = person0_eq0[0]
    person0_eq0_timeseries = person0_eq0[1]
    i = 0
    plt.figure(figsize=(2*20,(len(person0_eq0_timeseries.columns)-2+1)/2*5))
    for k in person0_eq0_timeseries:
        if k != 'Time' and k != 'Millis':
            plt.subplot((len(person0_eq0_timeseries.columns)-2+1)/2,2,i+1)
            plt.xlabel('Time')
            plt.ylabel(k)
            plt.plot(person0_eq0_timeseries['Time'].to_numpy(), person0_eq0_timeseries[k].to_numpy())
    ...
    for i in range(len(samples)):
        ...
        for k in range(len(sample_tp)):
            ...
            if len(resampled[k]) != DESIRED_SAMPLE_LENGTH:
                raise ValueError(f'expected a sample length of {DESIRED_SAMPLE_LENGTH}, but found length {len(resampled[k])}')
        resampled = np.array(resampled)
        samples[i] = np.transpose(resampled)
...
def load_data():
    ...
    if file_exists('preprocessed/2021'):
        with open('preprocessed/2021', 'rb') as f:
            (samples, labels) = pickle.load(f)
    else:
        if not file_exists(super_folder+pkl_filename):
            read_and_pickle_data()
        data = unpickle_data()
#         plot_data(data)
        ...
        with open('preprocessed/2021', 'wb') as f:
